...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *directory, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
        ...
        // while (n_folder--) {
            ...
            if (n < 0) {
                ...
                // return 1;
                    ...
                    length = sprintf(write,"%s/%s", namelist_folder[i]->d_name,(s = remove_ext (namelist[j]->d_name, '_', '/')));free (s);
                    length = sprintf(input,"%s/%s/%s", directory,namelist_folder[i]->d_name,namelist[j]->d_name);
                    free(namelist[j]);
                    ++j;
                    ...
                    draw_detections_my(im, dets, nboxes, thresh, names, alphabet, l.classes, write);
                    free_detections(dets, nboxes);
